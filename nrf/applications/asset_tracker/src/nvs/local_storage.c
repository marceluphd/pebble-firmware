...
#define LOCAL_STORAGE_SECTOR_COUNT  4U
static struct nvs_fs __local_storage;


/*
** @brief: Initialize local storage(zephyr nvs subsystem
** #LOCAL_STORAGE_SECTOR_COUNT define how many pages used as local storage
*/
int iotex_local_storage_init(void) {
    ...
    __local_storage.sector_count = LOCAL_STORAGE_SECTOR_COUNT;

    /* Init nvs filesystem */
    if ((ret = nvs_init(&__local_storage, DT_FLASH_DEV_NAME))) {
